...
# New env vars:
- `INTERNAL_SERVER_IP` - Internal IP for the wireguard server. Used in server mode. Can be set to an IPv4 or IPv6 or both
- `ALLOCATED_RANGES` - The IP ranges allocated for the wireguard server and peers. Used in server mode. Should be set according to the INTERNAL_SERVER_IP (ie. if INTERNAL_SERVER_IP is an IPv4, ALLOCATED_RANGES should be an IPv4 range and vice versa)
...
services:
  wireguard:
    ...
    environment:
      ...
      - SERVER_INTERNAL_IP=10.13.13.1
      - ALLOCATED_RANGES=10.13.13.2-10.13.13.254 #optional
...
docker run -d \
  ...
  -e SERVER_INTERNAL_IP=10.13.13.1 \
  -e ALLOCATED_RANGES=10.13.13.2-10.13.13.254 `#optional` \
